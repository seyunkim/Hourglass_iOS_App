...
class HAVideoPlayerScrollView : UIScrollView, UIGestureRecognizerDelegate {
    // MARK: Internal Variables
    var videoView : HAVideoPlayerView?
    var restaurantView : HARestaurantPromoView?
    var informationView : HARestaurantInformationView?
    ...
    internal func initializationImplementation() {
        ...
        videoView = HAVideoPlayerView(frame: self.frame,
        ...
        restaurantView = HARestaurantPromoView(frame: CGRectMake(
            self.frame.origin.x,
        ...
        informationView = HARestaurantInformationView(frame: CGRectMake(
            0,
    ...
    func reloadButtons() {
        videoView?.reloadButtons()
        restaurantView?.reloadButtons()
        informationView?.reloadButtons()
    }
    
    func reloadTaps() {
        var frameN = self.frame
        frameN.origin.y = 0
        frameN.size.height *= 3
        let heatMapView = HAHeatmapView(frame: frameN)
        heatMapView.drawHeatMap("VideoPlayerController")
